...
class PRNNFunction(Function):
	@staticmethod
	def graph2net(ctx, intput):
		return input

	@staticmethod
	def forward(ctx, input, weights, bias, old_h, old_cell):
		outputs = None # prnn_cpp.forward(input, weights, bias, old_h, old_cell)
	...
	def backward(ctx, grad_h, grad_cell):
		d_old_h, d_input, d_weights, d_bias, d_old_cell = None # prnn_cpp.backward(grad_h, grad_cell, *ctx.saved_variables)
